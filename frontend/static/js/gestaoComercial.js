document.addEventListener("DOMContentLoaded", function () {
  ...
  const chartSub = document.getElementById("gc-chart-sub");
  ...
  function buildChartAuditMap(chart) {
    ...
    const map = {};
    const defaultMetric = analiseId === "7" ? "receita_comercio" : "receita_frete";
  ...
  fetch(`/api/gestao_comercial_data?analise=${analiseId}&${params.toString()}`)
    ...
    .then((data) => {
      ...
      if (chartSub && data.chart_subtitle) {
        chartSub.textContent = data.chart_subtitle;
      }
      const pills = window.BIWEB_resolveChartAuditPills
        ? window.BIWEB_resolveChartAuditPills("gestao", null, analiseId, data.audit_pills)
        : data.audit_pills;
      if (window.BIWEB_renderChartAuditPills) {
        window.BIWEB_renderChartAuditPills("gc-chart-audit-pills", pills);
      }
      if (!data.chart) {
