...
def executar_atualizacao_bd_sati(apartamento_id: int):
    ...
    try:
        ...
        sati_schema = os.getenv("SATI_SCHEMA", "c3332").strip()
        url_login = (configs.get("URL_LOGIN") or "").strip()
        cod_sati = re.search(r"/(c\d+)/?$", url_login.rstrip("/"), re.I)
        cod_sati = cod_sati.group(1) if cod_sati else "?"
        db.logar_progresso(
            apartamento_id,
            f"Banco SATI: {os.getenv('SATI_DATABASE_URL', '').split('/')[-1]} | schema: {sati_schema} | código: {cod_sati}",
        )
        ...
        nome_zip = base_robo.nome_zip_da_url(url_zip)
        if not nome_zip:
            url_login = (configs.get("URL_LOGIN") or "").strip()
            cod_sati = re.search(r"/(c\d+)/?$", url_login.rstrip("/"), re.I)
            cod_sati = cod_sati.group(1) if cod_sati else "c3332"
            nome_zip = f"SATI-{cod_sati}-atual.zip"
        db.logar_progresso(apartamento_id, f"Arquivo esperado: {nome_zip}")
