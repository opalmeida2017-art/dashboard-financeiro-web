...
def clear_sati_engine_cache() -> None:
    get_sati_engine.cache_clear()
...
def get_sati_url_codigo(apartamento_id: int | None = None) -> str:
    """Código cXXXX no SATI (configuracoes_robo ou URL_LOGIN)."""
    import re

    cod_re = re.compile(r"^(c\d+)$", re.I)

    try:
        from app.data.database import engine as app_engine
        from app.data.tenant import get_transportadora_id

        apt = apartamento_id or get_transportadora_id()
        with app_engine.connect() as conn:
            for chave in ("SATI_URL_CODIGO", "SATI_PG_SCHEMA"):
                row = conn.execute(
                    text(
                        """
                        SELECT valor FROM configuracoes_robo
                        WHERE apartamento_id = :apt AND chave = :chave
                        LIMIT 1
                        """
                    ),
                    {"apt": apt, "chave": chave},
            ...
            row = conn.execute(
                text(
                    """
                    SELECT valor FROM configuracoes_robo
                    WHERE apartamento_id = :apt AND chave = 'URL_LOGIN'
                    LIMIT 1
                    """
                ),
                {"apt": apt},
        ...
        if row and row[0]:
            ...
            m = re.search(r"/(c\d+)$", url, re.I) or re.search(r"(c\d+)", url, re.I)
            if m:
                return m.group(1).lower()
    except Exception:
        pass
    env_cod = os.getenv("SATI_URL_CODIGO", "").strip().lower()
    if env_cod and cod_re.match(env_cod):
        return env_cod
    return ""


def get_sati_schema(apartamento_id: int | None = None) -> str:
    """Schema SATI no PostgreSQL = código cXXXX da URL_LOGIN (ex.: c2910)."""
    cod = get_sati_url_codigo(apartamento_id)
    if cod:
        return cod
    return os.getenv("SATI_SCHEMA", getattr(config, "SATI_SCHEMA", "c3332")).strip().lower() or "c3332"
...
def fetch_embarcadores_catalog(apartamento_id: int) -> list[dict]:
    """Lista todos os embarcadores do cadastro SATI (tabela embarcador)."""
    import re
    ...
    if not re.match(r"^c\d+$", schema or "", re.I):
        return []
    sql = query_embarcadores_cadastro(schema)
    try:
        with get_sati_engine().connect() as conn:
            df = pd.read_sql_query(text(sql), conn)
    except Exception as exc:
        print(f"AVISO: fetch_embarcadores_catalog ({schema}): {exc}")
        return []
    if df.empty:
        return []
    ...
    nome_col = col_map.get("nomeembarcador")
    if not cod_col:
        return []
    out: list[dict] = []
    for _, row in df.iterrows():
        try:
            cod_num = int(float(row[cod_col]))
