...
import uuid
from app.utils.limpar_dados import limpar_dados_importados
...
def index():
    ...
    unidades_embarque = logic.get_unique_unidades_embarque(apartamento_id=apartamento_id_alvo)
    ...
    return render_template('dashboards/index.html',
                           ...
                           selected_unidade_embarque=filters['unidade_embarque'],
                           selected_embarcador=filters['embarcador'],
...
def visao_comercial():
    ...
    return render_template(
        'dashboards/visao_comercial_index.html',
...
def visao_comercial_analise(analise_id: int):
    ...
    return render_template(
        'dashboards/visao_comercial_analise.html',
        analise=analise,
...
def visao_bi(visao_key: str):
    ...
    filters = parse_filters(request.args)
    return render_template(
        'dashboards/visao_bi_index.html',
        visao_key=visao_key,
...
def visao_bi_analise(visao_key: str, analise_id: int):
    ...
    filters = parse_filters(request.args)
    return render_template(
        'dashboards/visao_bi_analise.html',
        visao_key=visao_key,
        visao_titulo=visao['titulo'],
        analise=meta,
        analise_id=analise_id,
...
def _iniciar_atualizacao_banco_sati(apartamento_id_alvo: int, *, allow_setup: bool = False):
    ...
    from infra.tenant_licensing.bi_tenant_runtime import resolve_tenant_slug_from_request

    tenant_slug = (
        get_slug()
        or resolve_tenant_slug_from_request(request)
        or (session.get("bi_tenant_slug") or "").strip().lower()
        or None
    )
    if tenant_slug:
        session["bi_tenant_slug"] = tenant_slug

    execution_mode = os.getenv('EXECUTION_MODE', 'async')
    if execution_mode == 'sync':
        ok = logic.executar_atualizacao_bd_sati(apartamento_id_alvo, tenant_slug=tenant_slug)
...
def instalacao():
    """Configuração inicial (credenciais SAT) — redireciona para admin."""
...
def gerenciar_usuarios():
    ...
    return render_template('admin/gerenciar_usuarios.html', users=users, current_apartment_logo=current_apartment_logo)
