...
def get_dashboard_summary(apartamento_id: int, start_date=None, end_date=None, placa_filter="Todos", filial_filter=None, tipo_negocio_filter="Todos", unidade_embarque_filter=None, embarcador_filter="Todos"):
    print(f">>> [LOGIC] Chamando get_dashboard_summary para o apartamento ID: {apartamento_id}")
    summary_data = dm.get_dashboard_summary(apartamento_id, start_date, end_date, placa_filter, filial_filter, tipo_negocio_filter, unidade_embarque_filter, embarcador_filter)
    print(f"<<< [LOGIC] Retornando dados do summary: {'Dados calculados' if summary_data else 'Vazio'}")
    return summary_data

def get_monthly_summary(apartamento_id: int, start_date=None, end_date=None, placa_filter="Todos", filial_filter=None, tipo_negocio_filter="Todos", unidade_embarque_filter=None, embarcador_filter="Todos"):
    print(f">>> [LOGIC] Chamando get_monthly_summary para o apartamento ID: {apartamento_id}")
    monthly_data = dm.get_monthly_summary(apartamento_id, start_date, end_date, placa_filter, filial_filter, tipo_negocio_filter, unidade_embarque_filter, embarcador_filter)
    print(f"<<< [LOGIC] Retornando dados mensais: {'DataFrame com dados' if not monthly_data.empty else 'DataFrame vazio'}")
    return monthly_data

def get_unique_plates_with_types(apartamento_id: int, tipo_negocio_filter: str = "Todos"):
    print(f">>> [LOGIC] Chamando get_unique_plates_with_types para o apartamento ID: {apartamento_id}")
    return dm.get_unique_plates_with_types(apartamento_id, tipo_negocio_filter)
...
def get_unique_unidades_embarque(apartamento_id: int):
    print(f">>> [LOGIC] Chamando get_unique_unidades_embarque para o apartamento ID: {apartamento_id}")
    return dm.get_unique_unidades_embarque(apartamento_id)

def get_unique_embarcadores(apartamento_id: int):
    print(f">>> [LOGIC] Chamando get_unique_embarcadores para o apartamento ID: {apartamento_id}")
    return dm.get_unique_embarcadores(apartamento_id)
...
def get_gestao_comercial_data(apartamento_id, analise_id, start_date, end_date, placa_filter, filial_filter, tipo_negocio_filter):
    from app.core import gestao_comercial as gc
...
def get_visao_bi_data(visao_key, analise_id, apartamento_id, start_date, end_date, placa_filter, filial_filter, tipo_negocio_filter):
    from app.core import visoes_bi as vb
...
def get_bi_audit_data(
    ...
    from app.core import bi_audit

    return bi_audit.get_bi_audit_data(
        apartamento_id,
        metric,
        start_date,
        end_date,
...
def executar_atualizacao_bd_sati(
    apartamento_id: int, tenant_slug: str | None = None
) -> bool:
    """Robô: envio BI no SATI + download zip + restore PostgreSQL."""
    from infra.tenant_licensing.bi_tenant_runtime import TENANTS_ROOT, prepare_robot_context
    from sati_integration.robos.coletor_atualizacao_bd import executar_atualizacao_bd_sati
    from app.data.tenant import default_transportadora_id

    ok_tenant = prepare_robot_context(tenant_slug=tenant_slug, apartamento_id=apartamento_id)
    if TENANTS_ROOT.is_dir() and not ok_tenant:
        db.logar_progresso(
            apartamento_id,
            "ERRO: tenant BI não identificado. Acesse pelo link /acesso/<slug>/ "
            "(ex.: /acesso/w-carlos/) e tente novamente.",
        )
        return False

    apartamento_id = int(default_transportadora_id())
    sati_db = (os.getenv("SATI_DATABASE_URL") or "").rstrip("/").split("/")[-1]
    db.logar_progresso(
        apartamento_id,
        f"Tenant: {os.getenv('BI_TENANT_SLUG', '—')} | banco exclusivo: {sati_db or '—'}",
    )
    print(
        f">>> [LOGIC] Atualização SATI apt={apartamento_id} "
        f"tenant={tenant_slug or os.getenv('BI_TENANT_SLUG', '')} db={sati_db}"
    )
...
def executar_painel_documentos_sati(
    ...
    *,
    ...
    from sati_integration.robos.coletor_painel_documentos import executar_painel_documentos_sati
